...
#endif
{
    ...
    {        
        uint8_t field_length = p_data[index];//第一个值是数据长度
        uint8_t field_type = p_data[index+1];//第二个是数据类型
...
//							}
				...
				case BLE_GAP_AD_TYPE_MANUFACTURER_SPECIFIC_DATA://0xff
					
							//static uint8_t  *beacon_buff;
				      beacon_temp = p_typedata->p_data;//本质上不操作p_typedata->p_data，赋给中间变量
					
			      	beacon_buff = (ble_beacon_data *)beacon_temp;//把自定义数据包的首地址赋给beacon_buff
				
							//ble_beacon_data *beacon_buff;//结构体指针声明
				
							if((beacon_buff->type == 0x02) && beacon_buff->dlen == 0x15)//判断是否为beacon数据
							{
								uint32_t u_index;
...
/**@breif ibeacon distancing calculate function. */
...
//                CNXN_MNGR_LOG("[CM]: Connection Request Timedout.\r\n");
            ...
            break;
        case BLE_GAP_EVT_ADV_REPORT://广播事件
        {
            ...
            {                
                ...
                // Initialize advertisement report for parsing.解析beacon数据包
